...
struct tj_time *tj_time_convert(int input_time)
{
	...
	input_time += 8 * 3600;
	int day = input_time / 60 / 60 / 24;
	result.tj_second = input_time % 60 ;
	result.tj_minute = (input_time % 3600 - result.tj_second) / 60;
	result.tj_hour = (input_time -day* (60 * 60 * 24) - result.tj_second - result.tj_minute * 60) / 3600;
	...
	while (day > 0) {
		result.tj_day++;
		if (result.tj_day > maxday(result.tj_year, result.tj_month)) {
			result.tj_day=1;
			result.tj_month++;
